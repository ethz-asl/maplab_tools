...
class WBsRGB:
  def __init__(self, path_to_models, gamut_mapping=2, upgraded=0):
    if upgraded == 1:
  ...
  def rgb_uv_hist(self, I):
    ...
    img_sz = sz[0] * sz[1]
    if img_sz > 202500:  # resize if it is larger than 450*450
      factor = np.sqrt(202500.0 / img_sz)  # rescale factor
  ...
  def correctImage(self, I):
    ...
    hist = self.rgb_uv_hist(I)
    feature = self.encode(hist)
    # Do
    # ```python
