...
bool LocalizationHandler::processLocalizationResult(
  ...
  if (success) {
    fused_localization_result->timestamp_ns = localization_result->timestamp_ns;
    fused_localization_result->localization_mode =
        localization_result->localization_mode;
    fused_localization_result->T_G_M = T_G_M_;
    fused_localization_result->is_T_G_M_set = true;
    // TODO(mfehr): this should probably be the current fused T_G_M * T_M_I at
    // this time. With the current hack this is equal to the raw localization,
    // since T_G_M is derived based on T_G_B (most recent localization) * T_B_M
    // (odom pose at same time).
    fused_localization_result->T_G_B = localization_result->T_G_B;
...
bool LocalizationHandler::initializeBaseframe(
  ...
  if (T_M_B_buffer_.getPoseAt(localization_result.timestamp_ns, &T_M_B) >
  ...
  const aslam::Transformation T_G_M_loc_estimate =
      localization_result.T_G_B * T_M_B.inverse();
...
bool LocalizationHandler::processVisualLocalizationResultAsUpdate(
  ...
  const vio_common::PoseLookupBuffer::ResultStatus lookup_result =
      T_M_B_buffer_.getPoseAt(localization_result.timestamp_ns, &T_M_B);
  CHECK(
      lookup_result !=
  ...
  const double gravity_error_angle_deg =
      getLocalizationResultGravityDisparityAngleDeg(localization_result, T_M_B);
  ...
  if (FLAGS_use_6dof_localization) {
    // TODO(mfehr): TODO(LBern): fuse 6DoF localization results.
    measurement_accepted = true;
    T_G_M_ = localization_result.T_G_B * T_M_B.inverse();
  } else {
