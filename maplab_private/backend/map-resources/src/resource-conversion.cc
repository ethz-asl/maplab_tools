...
void convertDetectionToArtifact(const vi_map::ArtifactMeasurement& detection, 
  ...
  artifact->confidence = detection.getDetectionProbability();
  artifact->class_id = detection.getDetectedClassLabel();
  artifact->timestamp = detection.getTimestampNs();

  // Copy the transformation to the artifact.
  // We will store in a simple std::vector to ease de-/serialization.
  std::vector<double>& T_S_Artifact = artifact->T_S_Artifact;
  const std::size_t n_components = 7u; // [w x y z, x y z]
  T_S_Artifact.resize(n_components); 
  Eigen::Matrix<double, 7, 1>::Map(&T_S_Artifact[0], n_components) 
    = detection.getArtifactLocation().asVector();
}
