...
#include <map-resources/resource_artifact_detection.pb.h>
...
void ResourceLoader::saveResourceToFile(
    const std::string& file_path, const ResourceType& type,
    const resources::Artifact& resource) const {
  CHECK(!file_path.empty());
  CHECK(!common::fileExists(file_path));
  CHECK(common::createPathToFile(file_path));

  CHECK(type == backend::ResourceType::kArtifactDetection)
      << "The type '" << backend::ResourceTypeNames[static_cast<int>(type)]
      << "' is not of data type Artifact!";
  ...
  resources::proto::ArtifactDetections artifact_detections;
  artifact_detections.mutable_artifact_detection()->Reserve(1);

  resources::proto::ArtifactDetection* artifact_detection_ptr =
    artifact_detections.add_artifact_detection();

  artifact_detection_ptr->set_timestamp(resource.timestamp);
  resources::proto::ObjectInstanceBoundingBox *object_bbox_proto_ptr
     = new resources::proto::ObjectInstanceBoundingBox;
  object_bbox_proto_ptr->set_confidence(resource.confidence);
  object_bbox_proto_ptr->set_class_name(resource.class_id);
  artifact_detection_ptr->set_allocated_object_bbox(object_bbox_proto_ptr);
  *artifact_detection_ptr->mutable_transformation() 
    = {resource.T_S_Artifact.begin(), resource.T_S_Artifact.end()};


  constexpr bool kParseAsTextFormat = true;
  CHECK(common::proto_serialization_helper::serializeProtoToFile(
      folder_path, file_name, artifact_detections, kParseAsTextFormat))
      << "Failed to write the resource of data type "
      << "Artifact Detections to proto file '" << file_path << "'";
}

template <>
bool ResourceLoader::loadResourceFromFile(
    const std::string& file_path, const ResourceType& type,
    resources::Artifact* resource) const {
  CHECK_NOTNULL(resource);
  CHECK(!file_path.empty());
  if (!common::fileExists(file_path)) {
    VLOG(1) << "Resource file does not exist! Path: " << file_path;
  ...
  constexpr bool kParseAsTextFormat = true;
  if (!common::proto_serialization_helper::parseProtoFromFile(
          folder_path, file_name, &artifact_detections,
          kParseAsTextFormat)) {
    LOG(ERROR) << "Failed to read the resource of data type "
               << "Artifact Detection from the proto file '"
               << file_path << "'";
    return false;
  }

  // TODO(lbern): add multiple detections as resource
  //resource->resize(object_instance_bboxes.object_instance_bbox_size());

  //resources::ObjectInstanceBoundingBox& bbox = (*resource)[idx];
  // We only have one for now.
  const resources::proto::ArtifactDetection& artifact_detection =
      artifact_detections.artifact_detection(0);

  resource->timestamp = artifact_detection.timestamp();
  const resources::proto::ObjectInstanceBoundingBox& object_bbox_proto
    = artifact_detection.object_bbox();
  resource->confidence = object_bbox_proto.confidence();
  resource->class_id = object_bbox_proto.class_name();

  // Copy the transformation.
  const std::size_t n_components = artifact_detection.transformation_size();
  for (std::size_t i = 0u; i < n_components; ++i) {
    resource->T_S_Artifact.emplace_back(artifact_detection.transformation(i));
  }
