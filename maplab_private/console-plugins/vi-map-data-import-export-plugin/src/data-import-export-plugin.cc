...
DEFINE_string(
    artifact_detection_export_folder, "",
    "Folder to export the artifact detections into.");
...
DataImportExportPlugin::DataImportExportPlugin(common::Console* console)
  ...
  addCommand(
      {"export_all_artifact_detections", "eaad"},
      [this]() -> int { return exportArtifactDetections(); },
      "Exports the artifact detectinos to the folder specified with "
      "--artifact_detection_export_folder.",
      common::Processing::Sync);
}
...
int DataImportExportPlugin::getAllMissionIds(
...
int DataImportExportPlugin::exportArtifactDetections() const {
  std::string selected_map_key;
  if (!getSelectedMapKeyIfSet(&selected_map_key)) {
    return common::kStupidUserError;
  }

  if (FLAGS_artifact_detection_export_folder.empty()) {
    LOG(ERROR) << "Specify a valid export folder with "
               << "--artifact_detection_export_folder.";
    return common::kStupidUserError;
  }
  ...
  vi_map::MissionIdList mission_ids;
  int success = getAllMissionIds(map, &mission_ids);
  if (success != common::kSuccess) {
    VLOG(1) << "was not success";
    return success;
  }
  data_import_export::exportArtifactDetections(mission_ids,
      *map, FLAGS_artifact_detection_export_folder);
