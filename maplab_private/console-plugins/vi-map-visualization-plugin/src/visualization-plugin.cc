...
VisualizationPlugin::VisualizationPlugin(common::Console* console)
  ...
  addCommand(
      {"visualize_artifact_detections"},
      [this]() -> int { return visualizeArtifactDetections(); },
      "Visualizes the attached artifact detections.",
      common::Processing::Sync);
}
...
int VisualizationPlugin::visualizeArtifactDetections() const {
  std::string selected_map_key;
  if (!getSelectedMapKeyIfSet(&selected_map_key)) {
    return common::kStupidUserError;
  }
  ...
  visualization::visualizeReprojectedArtifacts(mission_ids, *map);
