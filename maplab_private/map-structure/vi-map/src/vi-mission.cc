...
VIMission::VIMission(const VIMission& other)
    : Mission(other),
      ...
      relative_6dof_id_(other.relative_6dof_id_),
      detection_id_(other.detection_id_),
...
void VIMission::setRelative6DoFSensor(const aslam::SensorId& relative_6dof_id) {
  CHECK(relative_6dof_id.isValid());
  relative_6dof_id_ = relative_6dof_id;
}

void VIMission::setArtifactDetectionSensor(const aslam::SensorId& detection_id) {
  CHECK(detection_id.isValid());
...
const aslam::SensorId& VIMission::getRelative6DoFSensor() const {
  CHECK(relative_6dof_id_.isValid())
      << "There is no valid Relative 6DoF Sensor ID associated with mission "
      << mission_id_.shortHex() << "!";
  return relative_6dof_id_;
}

const aslam::SensorId& VIMission::getArtifactDetectionSensor() const {
  CHECK(detection_id_.isValid())
      << "There is no valid Artifact Detection Sensor ID associated with mission "
      << mission_id_.shortHex() << "!";
...
bool VIMission::hasRelative6DoFSensor() const {
  return relative_6dof_id_.isValid();
}

bool VIMission::hasArtifactDetectionSensor() const {
  return detection_id_.isValid();
}
...
void VIMission::serialize(vi_map::proto::Mission* proto) const {
  ...
  if (relative_6dof_id_.isValid()) {
    relative_6dof_id_.serialize(proto->mutable_relative_6dof_id());
  }
  if (detection_id_.isValid()) {
    detection_id_.serialize(proto->mutable_detection_id());
  }
...
void VIMission::deserialize(
  ...
  if (proto.has_relative_6dof_id()) {
    relative_6dof_id_.deserialize(proto.relative_6dof_id());
  }
  if (proto.has_detection_id()) {
    detection_id_.deserialize(proto.detection_id());
  }
