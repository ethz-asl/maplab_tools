...
bool SensorManager::deserialize(const YAML::Node& yaml_node) {
  ...
  if (extrinsics_node.IsDefined() && !extrinsics_node.IsNull()) {
    ...
    for (const YAML::Node& extrinsic_node : extrinsics_node) {
      ...
      Eigen::Matrix4d T_B_S_matrix;
      if (!YAML::safeGet(
              extrinsic_node, static_cast<std::string>(kYamlFieldNameT_B_S),
              &T_B_S_matrix)) {
        LOG(ERROR) << "Unable to find T_B_S for extrinsics.";
        return false;
      }
      aslam::Transformation T_B_S(T_B_S_matrix);
    ...
    for (const YAML::Node& sensor_node : sensors_node) {
      ...
      switch (sensor_type) {
        ...
        case SensorType::kOdometry6DoF:
          ...
          break;
        case SensorType::kRelative6DoF:
          sensor = std::dynamic_pointer_cast<aslam::Sensor>(
              aligned_shared<vi_map::Relative6DoF>());
          break;
        ...
        case SensorType::kGpsUtm:
          ...
          break;
        case SensorType::kArtifactDetection:
          sensor = std::dynamic_pointer_cast<aslam::Sensor>(
              aligned_shared<vi_map::ArtifactDetection>());
          break;
...
bool SensorManager::isEqual(
  ...
  for (const std::pair<aslam::SensorId, aslam::Sensor::Ptr> sensor_pair :
       sensors_) {
  ...
  for (const std::pair<aslam::SensorId, aslam::SensorId> base_sensor_pair :
       base_sensor_id_map_) {
  ...
  for (const std::pair<aslam::SensorId, aslam::Transformation>
