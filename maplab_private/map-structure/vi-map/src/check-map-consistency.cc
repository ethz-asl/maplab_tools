...
bool checkSensorConsistency(const vi_map::VIMap& vi_map) {
  ...
  for (const MissionId& mission_id : mission_ids) {
    ...
    if (mission.hasRelative6DoFSensor()) {
      const aslam::SensorId& relative_6dof_id = mission.getRelative6DoFSensor();
      if (!sensor_manager.hasSensor(relative_6dof_id)) {
        LOG(ERROR)
            << "Mission " << mission_id.hexString()
            << " is assigned to a Relative 6Dof sensor that is not in the "
               "sensor manager!";
        return false;
      }
    }
