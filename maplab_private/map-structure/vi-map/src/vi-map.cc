...
bool VIMap::mergeAllMissionsFromMapWithoutResources(
  ...
  for (const std::pair<MissionId, pose_graph::VertexIdList>
...
std::string VIMap::printMapStatistics(
  ...
  // Print relative 6dof sensor/constraint info if present.
  if (mission.hasRelative6DoFSensor()) {
    aslam::SensorId relative_6dof_sensor_id = mission.getRelative6DoFSensor();
    // const Relative6DoF& relative_6dof =
    //     sensor_manager_.getSensor<Relative6DoF>(relative_6dof_sensor_id);
    stats_text << std::endl;
    print_aligned(
        "Relative-6DoF Sensor: ", relative_6dof_sensor_id.hexString(), 1);

    print_aligned(
        " - Relative-6DoF Edges: ", std::to_string(num_wheel_odometry_edges),
        1);
  }
  ...
  // Print Artifact sensor/constraint info if present
  if (mission.hasArtifactDetectionSensor()) {
    aslam::SensorId artifact_id = mission.getArtifactDetectionSensor();
    stats_text << std::endl;
    print_aligned("Artifact Detection Sensor: ", artifact_id.hexString(), 1);
  }
...
void VIMap::associateMissionSensors(
  ...
  for (const aslam::SensorId& sensor_id : sensor_ids) {
    ...
    } else if (sensor_type == SensorType::kArtifactDetection) {
      CHECK(!mission.hasArtifactDetectionSensor())
          << "Can not handle more than one artifact detection sensor per mission";
      mission.setArtifactDetectionSensor(sensor_id);
    } else if (sensor_type == SensorType::kRelative6DoF) {
      CHECK(!mission.hasRelative6DoFSensor())
          << "Can not handle more than one relative 6DOF sensor per mission";
      mission.setRelative6DoFSensor(sensor_id);
    } else if (sensor_type == SensorType::kArtifactDetection) {
      CHECK(!mission.hasArtifactDetectionSensor())
          << "Can not handle more than one artifact detection sensor per mission";
      mission.setArtifactDetectionSensor(sensor_id);
...
void VIMap::getAllAssociatedMissionSensorIds(
  ...
  if (mission.hasRelative6DoFSensor()) {
    sensor_ids->insert(mission.getRelative6DoFSensor());
  }
}
...
const vi_map::Relative6DoF& VIMap::getMissionRelative6DoFSensor(
    const vi_map::MissionId& id) const {
  return sensor_manager_.getSensor<vi_map::Relative6DoF>(
      getMission(id).getRelative6DoFSensor());
}

vi_map::Relative6DoF::Ptr VIMap::getMissionRelative6DoFSensorPtr(
    const vi_map::MissionId& id) const {
  return sensor_manager_.getSensorPtr<vi_map::Relative6DoF>(
      getMission(id).getRelative6DoFSensor());
}

const vi_map::ArtifactDetection& VIMap::getMissionArtifactDetection(
    const vi_map::MissionId& id) const {
  return sensor_manager_.getSensor<vi_map::ArtifactDetection>(
      getMission(id).getArtifactDetectionSensor());
}

vi_map::ArtifactDetection::Ptr VIMap::getMissionArtifactDetectionPtr(
    const vi_map::MissionId& id) const {
    return sensor_manager_.getSensorPtr<vi_map::ArtifactDetection>(
      getMission(id).getArtifactDetectionSensor());
}
...
void VIMap::mergeNeighboringVertices(
  ...
  const size_t num_outgoing_odometry_edges = mergeEdgesOfNeighboringVertices<
...
const vi_map::MissionId VIMap::duplicateMission(
  ...
  if (source_mission.hasRelative6DoFSensor()) {
    const aslam::SensorId source_sensor_id =
        source_mission.getRelative6DoFSensor();

    vi_map::Relative6DoF* cloned_relative_sensor = CHECK_NOTNULL(
        getMissionRelative6DoFSensor(source_mission_id).cloneWithNewIds());
    const aslam::SensorId new_sensor_id = cloned_relative_sensor->getId();
    const aslam::Transformation& T_B_S =
        getSensorManager().getSensor_T_B_S(source_sensor_id);
    const aslam::SensorId& base_sensor_id =
        getSensorManager().getBaseSensorId(source_sensor_id);
    getSensorManager().addSensor<vi_map::Relative6DoF>(
        vi_map::Relative6DoF::UniquePtr(cloned_relative_sensor), base_sensor_id,
        T_B_S);

    CHECK(mission_ptr);
    mission_ptr->setRelative6DoFSensor(new_sensor_id);
  }
  
  if (source_mission.hasArtifactDetectionSensor()) {
    const aslam::SensorId source_sensor_id = source_mission.getArtifactDetectionSensor();

    vi_map::ArtifactDetection* cloned_detection =
        CHECK_NOTNULL(getMissionArtifactDetection(source_mission_id).cloneWithNewIds());
    const aslam::SensorId new_sensor_id = cloned_detection->getId();
    const aslam::Transformation& T_B_S =
        getSensorManager().getSensor_T_B_S(source_sensor_id);
    const aslam::SensorId& base_sensor_id =
        getSensorManager().getBaseSensorId(source_sensor_id);
    getSensorManager().addSensor<vi_map::ArtifactDetection>(
        vi_map::ArtifactDetection::UniquePtr(cloned_detection), 
        base_sensor_id, T_B_S);

    CHECK(mission_ptr);

    mission_ptr->setArtifactDetectionSensor(new_sensor_id);
  }
