...
void VIMapEdgeMergingTest::addWheelOdometryEdges() {
  ...
  Relative6DoF::UniquePtr wheel_odometry_sensor(new Relative6DoF());
  ...
  T_B_S.setIdentity();
  map_.getSensorManager().addSensor<Relative6DoF>(
      std::move(wheel_odometry_sensor), wheel_odometry_sensor_id, T_B_S);
  CHECK(wheel_odometry_sensor_id.isValid());
  map_.getMission(mission_id_).setRelative6DoFSensor(wheel_odometry_sensor_id);
