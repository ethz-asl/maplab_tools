...
constexpr const char* kRelative6DoFIdentifier = "RELATIVE_6DOF";
...
constexpr const char* kArtifactDetectionIdentifier = "ARTIFACT_DETECTION";
...
inline SensorType convertStringToSensorType(const std::string& sensor_string) {
  ...
  std::function<bool(const char*, const char*)> equals =
  ...
  } else if (equals(sensor_c_string, kRelative6DoFIdentifier)) {
  ...
  } else if (equals(sensor_c_string, kArtifactDetectionIdentifier)) {
