...
class MaplabCameraInfoPublisher {
 public:
  explicit MaplabCameraInfoPublisher(
 ...
 private:
  ...
  bool startPublishing(std_srvs::Empty::Request&, std_srvs::Empty::Response&);
  bool stopPublishing(std_srvs::Empty::Request&, std_srvs::Empty::Response&);

  bool retrieveCameraIntrinsics(
      const aslam::Camera& camera, double* fu, double* fv, double* cu,
      double* cv) const;
  bool retrieveDistortionParameters(
      const aslam::Camera& camera, double* k1, double* k2, double* k3,
      double* k4, std::string* type) const;
  void createAndPublishCameraInfo(
