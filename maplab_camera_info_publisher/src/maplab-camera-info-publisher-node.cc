...
DEFINE_double(
    image_scale_factor, 1.0, "Defines the scale of republished image.");
DEFINE_bool(
    start_service, false,
...
MaplabCameraInfoPublisher::MaplabCameraInfoPublisher(
...
bool MaplabCameraInfoPublisher::initializeServicesAndSubscribers() {
  ...
  boost::function<bool(std_srvs::Empty::Request&, std_srvs::Empty::Response&)>
      callback_start = boost::bind(
          &MaplabCameraInfoPublisher::startPublishing, this, _1, _2);
  services_.emplace_back(
      nh_.advertiseService(kStartServiceTopic, callback_start));

  boost::function<bool(std_srvs::Empty::Request&, std_srvs::Empty::Response&)>
      callback_stop =
          boost::bind(&MaplabCameraInfoPublisher::stopPublishing, this, _1, _2);
  services_.emplace_back(
      nh_.advertiseService(kStopServiceTopic, callback_stop));
  ...
  for (const std::pair<std::string, size_t>& topic_camidx :
    ...
    const std::string info_topic =
        camera.getTopic() + FLAGS_cam_info_topic_suffix;
...
void MaplabCameraInfoPublisher::imageCallback(
    const sensor_msgs::ImageConstPtr& image, std::size_t camera_idx) {
  if (!should_publish_) {
    return;
  }
  if (shouldProcess()) {
...
void MaplabCameraInfoPublisher::compressedImageCallback(
  ...
  try {
    if (FLAGS_compressed_image_encoding == sensor_msgs::image_encodings::BGR8) {
...
bool MaplabCameraInfoPublisher::retrieveCameraIntrinsics(
    const aslam::Camera& camera, double* fu, double* fv, double* cu,
    double* cv) const {
  CHECK(fu);
  CHECK(fv);
  CHECK(cu);
...
bool MaplabCameraInfoPublisher::retrieveDistortionParameters(
    const aslam::Camera& camera, double* k1, double* k2, double* k3, double* k4,
...
void MaplabCameraInfoPublisher::publishProcessed(
    const cv::Mat& img, const std::size_t camera_idx,
    const sensor_msgs::ImageConstPtr& orig_img) const {
  sensor_msgs::ImagePtr img_msg =
      cv_bridge::CvImage(orig_img->header, getEncoding(is_greyscale_), img)
          .toImageMsg();
...
void MaplabCameraInfoPublisher::processImage(cv::Mat& processed) {
  ...
  if (FLAGS_image_rotation_angle_deg != 0 &&
    ...
    const cv::Point2f src_center(processed.cols / 2.0F, processed.rows / 2.0F);
...
bool MaplabCameraInfoPublisher::shouldProcess() const {
  return FLAGS_image_scale_factor != 1.0 || FLAGS_republish_grayscale ||
