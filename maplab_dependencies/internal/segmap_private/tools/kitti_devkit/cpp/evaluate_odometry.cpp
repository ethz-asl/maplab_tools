...
bool eval (string result_sha) {
  ...
  string traj_dir = "/home/rdube/.segmap/trajectories/";
  string id = result_sha;
  string run = "27";
  int32_t i = 27;
  //string gt_dir         = "data/odometry/poses";
  //string result_dir     = "results/" + result_sha;
  string result_dir     = traj_dir + id + "/results";
  ...
  //for (int32_t i=11; i<22; i++) {
    ...
    // file name
    char file_name[256];
    printf(file_name,"%02d.txt",i);

    // read ground truth and result poses
    //vector<Matrix> poses_gt     = loadPoses(gt_dir + "/" + file_name);
    //vector<Matrix> poses_result = loadPoses(result_dir + "/data/" + file_name);

    vector<Matrix> poses_gt     = loadPoses(traj_dir + result_sha + "/" + run + "_gt_0.csv");
    vector<Matrix> poses_result = loadPoses(traj_dir + result_sha + "/" + run + "_0.csv");

    // plot status
    printf("Processing: %s, poses: %d/%d",file_name,poses_result.size(),poses_gt.size());
    ...
    saveSequenceErrors(seq_err,error_dir + "/" + id);
    ...
    //if (i<=15) {
      ...
      savePathPlot(poses_gt,poses_result,plot_path_dir + "/" + id);
