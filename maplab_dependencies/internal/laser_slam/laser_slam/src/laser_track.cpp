...
bool LaserTrack::areTimestampsInRange(
    const uint64_t lhs, const uint64_t rhs) const {
  if (lhs > rhs) return lhs - rhs <= ts_epsilon_ns_;
  else return rhs - lhs <= ts_epsilon_ns_;
}
...
void LaserTrack::processPoseAndLaserScan(const Pose& pose, const LaserScan& in_scan,
  ...
  //if (!areTimestampsInRange(pose.time_ns, in_scan.time_ns)) {
...
Pose* LaserTrack::findPose(const Time& timestamp_ns) {
  ...
  /*
  CHECK(areTimestampsInRange(timestamp_ns, pose_measurements_.back().time_ns)) 
    << "The requested time (" << timestamp_ns 
    << ") is not close to the pose measurements. Last pose time is "
  ...
  //} while (it != pose_measurements_.begin() 
      //&& areTimestampsInRange(timestamp_ns, it->time_ns));

  CHECK_EQ(it->time_ns, timestamp_ns) 
  //CHECK(areTimestampsInRange(timestamp_ns, it->time_ns)) 
